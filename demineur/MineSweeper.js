var MineSweeper = {
    ...
    checkPosition: function (x, y, check) {
        ...
        if (this.game.field[x][y] == -1) {
            document.getElementById('cell-' + x + '-' + y).className = 'cell bomb lost';
            this.displayLose();
            return;
        }
    ...
    displayLose: function () {
        for (i = 1; i <= this.settings['lines']; i++) {
            for (j = 1; j <= this.settings['columns']; j++) {
                if (document.getElementById('cell-' + i + '-' + j).className != 'cell bomb lost') {
                    if (this.game.field[i][j] == -1) {
                        document.getElementById('cell-' + i + '-' + j).className = 'cell bomb';
                    } else if (this.game.field[i][j] > 0) {
                        document.getElementById('cell-' + i + '-' + j).className = 'cell clear';
                        document.getElementById('cell-' + i + '-' + j).innerHTML = this.game.field[i][j];
                    } else {
                        document.getElementById('cell-' + i + '-' + j).className = 'cell clear';
                    }
                }
